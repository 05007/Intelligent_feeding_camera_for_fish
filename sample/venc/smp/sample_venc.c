...
void SAMPLE_VENC_HandleSig(SC_S32 signo)
{
    ...
    if (SIGINT == signo || SIGTERM == signo)
    {
...
#if 0
        if (__stage.vpss_bind)
            SAMPLE_COMM_VPSS_UnBind_VENC(0, 0, 0);

        if (__stage.venc_inited)
            SAMPLE_COMM_VENC_Stop(0);

        for (int i = 1; abChnEnable[i]; i++)
        {
            if (__stage.vpss_bind)
                SAMPLE_COMM_VPSS_UnBind_VENC(0, i, i);
        ...
        if (__stage.vpss_inited)
            SAMPLE_COMM_VPSS_Stop(0, abChnEnable);

        if (__stage.vi_bind_venc)
            SAMPLE_COMM_VI_UnBind_VENC(0, 0, 0);

        if (__stage.venc_inited)
            SAMPLE_COMM_VI_StopVi(&stViConfig);

        if (__stage.blk != 0xFFFFFFFF)
            SC_MPI_VB_ReleaseBlock(__stage.blk);

        if (__stage.poolId != 0xFFFFFFFF)
            SC_MPI_VB_MunmapPool(__stage.poolId);
#endif
...
SC_S32 SAMPLE_VENC_2CHN(void)
{
    ...
    stSize1[1].u32Width = 1920;
    stSize1[1].u32Height = 1080;
    stSize1[1].u32Width = 720;
    stSize1[1].u32Height = 576;

    PIC_SIZE_E ps[3] = {enPicSize, PIC_D1_PAL, PIC_D1_PAL};
...
SC_S32 SAMPLE_VENC_Jpeg(void)
{
	...
	stChnParam.stFrameRate.s32DstFrmRate = 5;
